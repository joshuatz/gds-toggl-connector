...
import {
    responseTemplate,
    TogglDetailedReportRequestParams,
    TogglSummaryReportRequestParams,
    TogglWeeklyReportRequestParams
} from './toggl-types';
...
export class TogglApi {
    ...
    getDetailsReportAllPages(
        workspaceId: number,
        since: Date,
        until: Date,
        filterToBillable?: boolean,
        startPage?: number
    ) {
        const MAX_PAGES = 30;
    ...
    getSummaryReport(
        ...
        since: Date,
        until: Date,
        grouping: 'projects' | 'clients' | 'users' = 'projects',
        subgrouping: 'time_entries' | 'tasks' | 'projects' | 'users' = 'time_entries',
        filterToBillable?: boolean
    ) {
        filterToBillable = typeof filterToBillable === 'boolean' ? filterToBillable : false;
        let filterToBillableString: 'yes' | 'both' | 'no' = filterToBillable == true ? 'yes' : 'both';
     ...
     */
    static requestParamsToQueryString(
        requestParams:
            | TogglDetailedReportRequestParams
            | TogglSummaryReportRequestParams
            | TogglWeeklyReportRequestParams,
        urlToAppendTo?: string
    ) {
